...
const AdminComplainTable = () => {
  ...
  const fetchTickets = async (status) => {
    try {
      ...
      const adminId = localStorage.getItem('adminId'); // Retrieve the logged-in admin's ID from local storage
  
      const response = await axios.get(`http://localhost:5044/api/NormalAdminDashboard/complaints/${statusId}`, {
        params: { adminId }, // Pass the admin ID as a query parameter
      });
  ...
  const handleTicketClick = (ticket) => {
    navigate('/normal-admin/AdminTicketPage', {
      state: {
        ticketId: ticket.tId,
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 shadow-lg max-w-sm w-full">
            ...
            <div className="flex justify-end">
              <button className="bg-gray-500 text-white px-4 py-2 rounded-md mr-2" onClick={handlePopupClose}>Cancel</button>
              <button className="bg-blue-500 text-white px-4 py-2 rounded-md" onClick={handleAssignSubmit}>Assign</button>
            </div>
          </div>
        </div>
      )}
